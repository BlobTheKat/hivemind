...
static bool _fire_pipe(hivemind_server_t* s, const uint32_t id[5], const uint8_t* data, size_t len){
	uint64_t hash = _mix64((uint64_t)id[0]<<32|id[4])^_mix64((uint64_t)id[2]<<32|id[3]);
	...
	if(i >= b){
		...
		if(!b){
			...
			for(size_t i = 0; i < ob; i++){
				...
				uint64_t hash = _mix64((uint64_t)oh[i].id[0]<<32|oh[i].id[4])^_mix64((uint64_t)oh[i].id[2]<<32|oh[i].id[3]);
...
static inline void _keyless_sig2(hivemind_server_t* s, const remote_t* from, const uint32_t knonce[restrict 5], uint32_t out_tag[restrict 8], uint32_t* restrict out_xor, unsigned xor_count){
	uint32_t chacha[16] = {0x2d6d6172, 0x6b636170, 0x6b206465, 0x68637865}; // "ram-packed kexch"
	memcpy(chacha+4, s->master_key, 32);
...
static uint64_t _ram_packed_kex_verify(hivemind_server_t* s, const remote_t* from, uint32_t out_key[restrict 8], const uint32_t in_packet[restrict 5], uint32_t pipeid[restrict 5]){
	_keyless_sig2(s, from, in_packet, out_key, pipeid, 5);
	uint64_t t = epoch_now()/MILLISECOND_US, t1 = (uint64_t)le32toh(in_packet[0])<<24|(uint64_t)(le32toh(in_packet[1])&0xFFFFFF);
	if((t>t1?t-t1:t1-t) > s->state_lifetime) return 0;
	uint32_t last[5];
	_nalloc_id(s, last);
...
static inline void _split_by_hash(struct _hivemind_remote *p, struct _hivemind_remote** out, size_t buckets){
	...
	while(p){
		...
		uint64_t hash = _mix64(((uint64_t)p->addr.dwords[0]<<32|p->addr.dwords[1]) ^ _mix64((uint64_t)p->addr.dwords[2]<<32|p->addr.dwords[3]) ^ (p->port * 0x9e3779b97f4a7c15ull));
...
static struct _hivemind_remote* _hivemind_state_find(hivemind_server_t* s, ip_addr_t addr, uint16_t port, bool create){
	uint64_t hash = _mix64(((uint64_t)addr.dwords[0]<<32|addr.dwords[1]) ^ _mix64((uint64_t)addr.dwords[2]<<32|addr.dwords[3]) ^ (port * 0x9e3779b97f4a7c15ull));
...
size_t read_len_inc(const uint8_t* payload){
...
void encrypt_packet(uint32_t state[16], struct _send_packet* p){
